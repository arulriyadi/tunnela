...
"""
import configparser, secrets, os, re, pyotp, ipaddress, bcrypt
...
class DashboardConfig:
        ...
        self.__default = {
            ...
            "Server": {
                ...
                "gunicorn_socket_path": os.path.join(
                    DashboardConfig.ConfigurationPath, "run", "gunicorn.sock"
                ),
    ...
    def getConnectionString(self, database) -> str or None:
        ...
        if self.GetConfig("Database", "type")[1] == "postgresql":
            cn = f'postgresql+psycopg://{self.GetConfig("Database", "username")[1]}:{self.GetConfig("Database", "password")[1]}@{self.GetConfig("Database", "host")[1]}/{database}'
    ...
    def validate_server_timezone(self, value: Any) -> tuple[bool, str]:
        """IANA name check for dashboard_timezone (also used before timedatectl)."""
        tz = (str(value) if value is not None else "").strip()
        if not tz:
            return False, "Timezone cannot be empty"
        if tz not in available_timezones():
            return False, "Invalid IANA timezone name"
        return True, ""

    @staticmethod
    def validate_app_listen_mode(value: Any) -> tuple[bool, str]:
        v = (str(value) if value is not None else "").strip()
        if v not in ("direct", "nginx_socket"):
            return False, 'app_listen_mode must be "direct" or "nginx_socket".'
        return True, ""

    @staticmethod
    def validate_gunicorn_socket_path(value: Any) -> tuple[bool, str]:
        """Absolute Unix path for Gunicorn Unix socket (nginx_socket mode)."""
        s = (str(value) if value is not None else "").strip()
        if not s:
            return False, "gunicorn_socket_path cannot be empty."
        if len(s) > 512:
            return False, "gunicorn_socket_path is too long (max 512 characters)."
        if not s.startswith("/"):
            return False, "gunicorn_socket_path must be an absolute path (start with /)."
        if ".." in s or "\x00" in s:
            return False, "Invalid characters in gunicorn_socket_path."
        parent = os.path.dirname(s)
        if parent and os.path.exists(parent):
            if not os.path.isdir(parent):
                return False, "gunicorn_socket_path parent must be a directory."
            if not os.access(parent, os.W_OK):
                return False, "gunicorn_socket_path parent directory is not writable."
        return True, ""

    @staticmethod
    def validate_systemd_unit(value: Any) -> tuple[bool, str]:
        """Optional systemd unit name for UI-triggered restart (empty = use wgd.sh only)."""
        s = (str(value) if value is not None else "").strip()
        if not s:
            return True, ""
        if len(s) > 128:
            return False, "systemd_unit is too long (max 128 characters)."
        if not re.match(r"^[a-zA-Z0-9._@-]+$", s):
            return False, "systemd_unit contains invalid characters."
        return True, ""
    ...
    def __configValidation(self, section : str, key: str, value: Any) -> tuple[bool, str]:
        ...
        if section == "Server" and key == "dashboard_timezone":
            return self.validate_server_timezone(value)
        if section == "Server" and key == "login_copyright_text":
            s = str(value) if value is not None else ""
            if len(s.strip()) == 0:
                return False, "Login copyright text cannot be empty."
            if len(s) > 600:
                return False, "Login copyright text must be at most 600 characters."
        if section == "Server" and key == "login_copyright_url":
            s = (str(value) if value is not None else "").strip()
            if s and not (s.startswith("http://") or s.startswith("https://")):
                return False, "Link URL must start with http:// or https:// (or be empty)."
        if section == "Server" and key == "app_listen_mode":
            return self.validate_app_listen_mode(value)
        if section == "Server" and key == "gunicorn_socket_path":
            return self.validate_gunicorn_socket_path(value)
        if section == "Server" and key == "systemd_unit":
            return self.validate_systemd_unit(value)
