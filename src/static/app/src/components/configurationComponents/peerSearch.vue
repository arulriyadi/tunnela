...
export default {
	...
	props: {
		...
		displayTags: Array,
		/** When true, use compact layout for embedding in page header (e.g. Clients). */
		embedded: {
			type: Boolean,
			default: false
		},
		/** When true, hide "Download All" (e.g. Clients — bulk download via Select Peers). */
		hideDownloadAll: {
			type: Boolean,
			default: false
		},
		/** Clients page: replicate tag definitions to every WireGuard configuration. */
		syncPeerGroupsToAll: {
			type: Boolean,
			default: false
		},
		syncPeerGroupsConfigurationNames: {
			type: Array,
			default: () => []
		},
		peerGroupsByConfiguration: {
			type: Object,
			default: () => ({})
		}
	},
	...
	methods: {
		...
		onPeerTagUpdate(args){
			this.configuration.Info.PeerGroups = args
			this.$emit('peer-groups-updated', args)
		}
	}
}
</script>
...
<template>
	<div class="d-flex flex-column gap-2" :class="embedded ? 'my-0' : 'my-4'">
		<div
			class="d-flex gap-2 peerSearchContainer"
			:class="{'flex-wrap justify-content-end': embedded}"
		>
			<div class="dropdown">
				<button
			...
			<div class="position-relative">
				...
				<Transition name="slide-fade">
					<PeerTag
						@update="onPeerTagUpdate"
						@close="this.tagManager = false"
						:configuration="configuration"
						:sync-peer-groups-to-names="syncPeerGroupsToAll && syncPeerGroupsConfigurationNames.length ? syncPeerGroupsConfigurationNames : null"
						:peer-groups-by-configuration="syncPeerGroupsToAll ? peerGroupsByConfiguration : null"
						v-if="this.tagManager"
					></PeerTag>
				</Transition>
			</div>
