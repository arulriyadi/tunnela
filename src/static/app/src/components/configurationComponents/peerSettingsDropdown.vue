...
export default {
	...
	computed: {
		configId(){
			return (this.ConfigurationInfo && this.ConfigurationInfo.Name) || this.$route.params.id
		}
	},
	data(){
		return{
	...
	methods: {
		downloadPeer(){
			fetchGet("/api/downloadPeer/"+this.configId, {
				id: this.Peer.id
			}, (res) => {
				if (res.status){
		...
		deletePeer(){
			...
			fetchPost(`/api/deletePeers/${this.configId}`, {
				peers: [this.Peer.id]
			}, (res) => {
		...
		restrictPeer(){
			...
			fetchPost(`/api/restrictPeers/${this.configId}`, {
				peers: [this.Peer.id]
			}, (res) => {
				this.dashboardStore.newMessage("Server", res.message, res.status ? "success":"danger")
				this.$emit("refresh")
		...
		allowAccessPeer(){
			...
			fetchPost(`/api/allowAccessPeers/${this.configId}`, {
				peers: [this.Peer.id]
			}, (res) => {
				this.dashboardStore.newMessage("Server", res.message, res.status ? "success":"danger")
				this.$emit("refresh")
