...
const props = defineProps({
	...
	/** When set (non-empty), POST PeerGroups to every listed configuration name. */
	syncPeerGroupsToNames: { type: Array, default: null },
	/** Map configuration Name → existing PeerGroups; used to keep Peers[] per interface when syncing. */
	peerGroupsByConfiguration: { type: Object, default: null },
})
const groups = reactive({...(props.configuration.Info.PeerGroups ?? {})})

function mergePeerGroupsPreservePeers(
	sourceGroups: Record<string, unknown>,
	previousGroups: Record<string, unknown> | null | undefined
) {
	const prev = previousGroups || {}
	const out: Record<string, unknown> = {}
	for (const key of Object.keys(sourceGroups)) {
		const src = sourceGroups[key] as Record<string, unknown>
		const p = prev[key] as { Peers?: unknown[] } | undefined
		out[key] = {
			...src,
			Peers: p && Array.isArray(p.Peers) ? [...p.Peers] : [],
		}
	}
	return out
}
...
watch(() => groups, () => {
	const serialized = JSON.parse(JSON.stringify(groups)) as Record<string, unknown>
	const targets =
		Array.isArray(props.syncPeerGroupsToNames) && props.syncPeerGroupsToNames.length
			? props.syncPeerGroupsToNames
			: [props.configuration.Name]

	let pending = targets.length
	let allOk = true

	const doneOne = (res: { status?: boolean } | undefined) => {
		if (!res?.status) {
			allOk = false
		}
		pending--
	...
	for (const targetName of targets) {
		let value: Record<string, unknown> = serialized
		if (
			props.peerGroupsByConfiguration &&
			targetName !== props.configuration.Name
		) {
			const prev = props.peerGroupsByConfiguration[targetName] as
				| Record<string, unknown>
				| undefined
			value = mergePeerGroupsPreservePeers(serialized, prev)
		}
		fetchPost(
