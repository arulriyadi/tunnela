...
export default {
	...
	async setup(){
		...
		const loginFooter = ref({text: "", url: ""});
		const footerFetch = fetchGet("/api/getLoginFooterSettings", {}, (res) => {
		...
		return {store, theme, totpEnabled, version, loginFooter}
	},
	data(){
		return {
			data: {
	...
	computed: {
		...
		loginFooterParts() {
			const text = this.loginFooter?.text ?? "";
			const url = (this.loginFooter?.url ?? "").trim();
			if (!url) {
...
<template>
	<div class="container-fluid login-container-fluid d-flex main flex-column py-4 text-body h-100" 
		...
		<small class="text-muted pb-3 d-block w-100 text-center mt-3">
			{{ loginFooterParts.prefix }}<a
				v-if="loginFooterParts.href"
				:href="loginFooterParts.href"
				target="_blank"
				rel="noopener noreferrer"
			><strong>{{ loginFooterParts.linkText }}</strong></a>
		</small>
