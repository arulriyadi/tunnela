...
# One-time: NOPASSWD for timedatectl set-timezone (Settings → Date & time) when Gunicorn is not root.
# Idempotent: skips if file already exists. Requires root; uses SUDO_USER when you ran sudo ./wgd.sh.
_ensure_wgdashboard_timedatectl_permissions() {
	command -v timedatectl >/dev/null 2>&1 || return 0
	TD="$(command -v timedatectl)"
	[ "$(id -u)" -eq 0 ] || return 0
	local SUDOERS_FILE="/etc/sudoers.d/wgdashboard-timedatectl"
	...
	printf '%s ALL=(root) NOPASSWD: %s set-timezone *\n' "$SUDO_USER" "$TD" > "$SUDOERS_FILE.tmp" || return 0
	if command -v visudo >/dev/null 2>&1; then
		if ! visudo -cf "$SUDOERS_FILE.tmp" 2>/dev/null; then
	...
	printf "[WGDashboard] %s Created %s (NOPASSWD: timedatectl set-timezone for user %s)\n" "$heavy_checkmark" "$SUDOERS_FILE" "$SUDO_USER"
}

# Nginx "Deploy to nginx" from Settings writes to /etc/nginx and runs nginx -t / reload.
# That requires the Gunicorn process to run as root — use: sudo ./wgd.sh start
# (see implementasi-nginx-gunicorn-socket.md → "Deploy konfigurasi Nginx dari UI" / "Izin").
# No automatic sudoers entry is added here (unlike timedatectl above).
...
# Fase 3: ensure parent dir for Unix socket exists before Gunicorn (same path family as wg-dashboard.ini).
# Requires an active venv (caller must run _check_and_set_venv first).
_precreate_gunicorn_unix_socket_parent () {
	if [[ -n "${CONFIGURATION_PATH}" ]]; then
		WGD_INI_PATH="${CONFIGURATION_PATH}/wg-dashboard.ini"
	else
		WGD_INI_PATH="./wg-dashboard.ini"
	fi
	if [[ ! -f "$WGD_INI_PATH" ]]; then
		return 0
	fi
	export WGD_INI_PATH
	sock_parent="$("$venv_python" -c "
import configparser, os
cfg = configparser.ConfigParser()
cfg.read(os.environ['WGD_INI_PATH'], encoding='utf-8')
if not cfg.has_section('Server'):
    raise SystemExit(0)
sec = cfg['Server']
mode = (sec.get('app_listen_mode', 'direct') or 'direct').strip()
if mode != 'nginx_socket':
    raise SystemExit(0)
path = (sec.get('gunicorn_socket_path', '') or '').strip()
if not path:
    raise SystemExit(0)
...
print(parent)
" 2>/dev/null)" || return 0
	if [[ -z "$sock_parent" ]]; then
		return 0
	fi
	if [[ ! -d "$sock_parent" ]]; then
		printf "[WGDashboard] Creating Unix socket directory: %s\n" "$sock_parent"
		sudo mkdir -p "$sock_parent"
		sudo chmod 755 "$sock_parent" || true
	fi
}
